...
const MessageReply: React.FC<MessageReplyProps> = ({ replyTo, onCancel }) => {
    ...
    return (
        <div className="flex items-start space-x-2 px-3 py-2bg-gray-50 dark:bg-dark-800 border-l-2 border-primary-600 rounded">
            <div className="flex-1 min-w-0">
                <div className="text-xs text-primary-400 font-medium mb-1">
                    Replying to {getAuthorName()}
                </div>
                <div className="text-sm text-gray-600 dark:text-dark-300 truncate">
                    {replyTo.content}
                </div>
            </div>
            {onCancel && (
                <button
                    onClick={onCancel}
                    className="flex-shrink-0 p-1 hover:bg-gray-100 dark:hover:bg-dark-700 rounded transition"
                >
