        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-dark-800 rounded-lg max-w-3xl w-full max-h-[80vh] overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-dark-700 flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Choose what to share</h3>
                    <button
                        onClick={onCancel}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {sources.map((source) => (
                            <button
                                ...
                                className="bg-dark-700 rounded-lg p-2 hover:bg-gray-200 dark:hover:bg-dark-600 transition border-2 border-transparent hover:border-primary-500 text-left"
                            >
                                <img
                                    src={source.thumbnail}
                                    alt={source.name}
                                    className="w-full aspect-video object-cover rounded mb-2 bg-white dark:bg-dark-900"
                                />
                                <p className="text-sm text-gray-900 dark:text-white truncate">{source.name}</p>
                            </button>
                        ))}
                    </div>
                </div>
