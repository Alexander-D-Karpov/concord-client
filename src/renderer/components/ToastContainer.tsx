...
const Toast: React.FC<{ toast: ToastNotification; onDismiss: () => void }> = ({ toast, onDismiss }) => {
    ...
    const getIcon = () => {
        switch (toast.type) {
            ...
            default:
                return (
                    <div className="w-10 h-10bg-gray-200 dark:bg-dark-600 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white font-semibold">{toast.avatarInitial || '#'}</span>
                    </div>
                );
        }
    ...
    return (
        <div
            onClick={handleClick}
            className="bg-dark-800 border border-gray-200 dark:border-dark-600 rounded-lg shadow-2xl p-4 flex items-start space-x-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-700 transition-all duration-200 animate-slide-in max-w-sm"
        >
            {getIcon()}
            <div className="flex-1 min-w-0">
                <div className="font-semibold text-gray-900 dark:text-white text-sm truncate">{toast.title}</div>
                <div className="text-dark-300 text-sm truncate">{toast.body}</div>
            </div>
            <button
                onClick={(e) => { e.stopPropagation(); onDismiss(); }}
                className="flex-shrink-0 p-1 hover:bg-gray-200 dark:hover:bg-dark-600 rounded transition"
            >
