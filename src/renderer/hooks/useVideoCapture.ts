...
function ensureSPSPPS(data: Uint8Array, isKeyframe: boolean): Uint8Array {
    if (!isKeyframe) return data;

    const nalus = parseAnnexBNALUs(data);
    const sps = nalus.find(n => (n[0] & 0x1f) === 7);
    const pps = nalus.find(n => (n[0] & 0x1f) === 8);

    if (sps) cachedSPS = sps;
    if (pps) cachedPPS = pps;

    if (sps && pps) return data;
    if (!cachedSPS || !cachedPPS) return data;

    const startCode = new Uint8Array([0, 0, 0, 1]);
    const parts: Uint8Array[] = [];
    if (!sps) { parts.push(startCode); parts.push(cachedSPS); }
    if (!pps) { parts.push(startCode); parts.push(cachedPPS); }
    parts.push(data);
...
export function useVideoCapture(enabled: boolean, isScreenShare: boolean, options: VideoCaptureOptions = {}) {
    ...
    const connectionQualityRef = useRef<number>(3);
    const currentConfigRef = useRef<VideoEncoderConfig | null>(null);
    ...
    const createEncoder = useCallback(async (
        ...
        const qualityMultiplier = connectionQualityRef.current >= 3 ? 1.0
            : connectionQualityRef.current === 2 ? 0.6 : 0.3;
        let targetBitrate = Math.floor(
            bitrateForResolution(actualWidth, actualHeight, actualFps, isScreenShare) * qualityMultiplier
        );
        ...
        const encoder = new VideoEncoderCtor({
            output: (chunk: EncodedVideoChunk) => {
                ...
                if (isH264) data = ensureSPSPPS(data, isKf);
                if (isKf) lastKeyframeAt = frameCount;
                window.concord?.sendVoiceVideo?.(data.buffer, isKf, isScreenShare ? 'screen' : 'camera');
            },
        ...
        const processFrames = async () => {
            while (running && mountedRef.current) {
                try {
                    ...
                    if (Date.now() - lastBitrateCheck > BITRATE_CHECK_INTERVAL) {
                        lastBitrateCheck = Date.now();
                        const qMul = connectionQualityRef.current >= 3 ? 1.0
                            : connectionQualityRef.current === 2 ? 0.6 : 0.3;
                        const newBitrate = Math.floor(
                            bitrateForResolution(actualWidth, actualHeight, actualFps, isScreenShare) * qMul
                        );
                        if (newBitrate !== targetBitrate && encoder.state === 'configured' && currentConfigRef.current) {
                            try {
                                const updated = { ...currentConfigRef.current, bitrate: newBitrate };
                                encoder.configure(updated);
    ...
    useEffect(() => {
        const unsub = window.concord.onVoiceQuality?.((data: any) => {
            const q = data.local ?? 3;
            const prev = connectionQualityRef.current;
            connectionQualityRef.current = q;

            if (q !== prev && encoderRef.current && encoderRef.current.state === 'configured' && currentConfigRef.current) {
                const multiplier = q >= 3 ? 1.0 : q === 2 ? 0.6 : 0.3;
                const track = stream?.getVideoTracks()[0];
                const settings = track?.getSettings();
                const w = settings?.width || width;
                const h = settings?.height || height;
                const fps = isScreenShare ? 15 : (settings?.frameRate || frameRate);
                const newBitrate = Math.floor(bitrateForResolution(w, h, fps, isScreenShare) * multiplier);

                if (newBitrate !== currentConfigRef.current.bitrate) {
                    try {
                        const updated = { ...currentConfigRef.current, bitrate: newBitrate };
                        encoderRef.current.configure(updated);
                        currentConfigRef.current = updated;
                    } catch {}
                }
            }
        ...
        return () => unsub?.();
    }, [stream, isScreenShare, width, height, frameRate]);
